...
const getFreshnessStatus = (lastUpdate: Date | null, isRealtime: boolean) => {
  if (!lastUpdate) return { status: 'unknown', color: 'bg-muted', label: 'Unknown' };
  ...
  if (ageSec < 15) return { status: 'fresh', color: 'bg-emerald-500', label: 'Fresh' };
  if (ageSec < 45) return { status: 'ok', color: 'bg-emerald-400', label: 'OK' };
  if (ageSec < 90) return { status: 'stale', color: 'bg-amber-500', label: 'Stale' };
  return { status: 'outdated', color: 'bg-destructive', label: 'Outdated' };
};
