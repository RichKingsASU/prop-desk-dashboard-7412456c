...
export function useDataFreshness() {
  ...
  const fetchFreshness = async () => {
    // Supabase-based telemetry is disabled; show unknown/empty state.
    setTables((prev) => prev.map((t) => ({ ...t, lastRowTimestamp: null, rowCountLast15Min: 0, status: 'unknown', loading: false })));
    setJobs([]);
    setLoading(false);
